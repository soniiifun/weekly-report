...
5. tasks: 該專案底下的工作項目。每個 task 請產生一個唯一的 id (例如 "t1", "t2")。【重要規則】：請在每個「任務名稱」(description) 的最前面，統一加上 [專案名稱] 的中括號標籤。例如：如果專案名稱是「SABA濾水器」，任務是「確認上線時間」，則 description 必須是「[SABA濾水器] 確認上線時間」。
